...
* Внутри окна каждая группа, объединённая по одинаковым параметрам (`$params`):
  * Сортируется в обратном порядке по времени добавления;
  * И ранжируется по принципу: "чем новее документ, тем выше его ранг (=> меньше его номер)".
* Поэтому, фильтруя в итоге документы по рангу 1, мы выбираем самый свежий документ из каждой группы
  * (то есть выбираем уникальные и самые последние документы).
* Остаётся только:
  * Отсортировать эти уникальные документы по убыванию;
  * И выбрать из них только 5 самых свежих.